...
function OtpSignupLogic() {
  ...
  const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      const response = await fetch("https://fastapi-turbine-62vm.onrender.com/api/v1/auth/verify-registration", {
        method: "POST",
        headers: {
  ...
  const handleResend = async () => {
    ...
    try {
      const response = await fetch("https://fastapi-turbine-62vm.onrender.com/api/v1/auth/resend-registration-otp", {
        credentials: "include",
        method: "POST",
